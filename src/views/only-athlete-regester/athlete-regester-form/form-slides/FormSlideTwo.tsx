...
const FormSlideTwo = () => {
  ...
  const initialFormData = getLocalFormData("slideTwoFormData") || {
  ...
  useEffect(() => {
    const { addressProof, ...restData} = formData;
    setLocalFormData("slideTwoFormData", restData);
    updateFormData();
  }, [formData]);
  ...
  const handleChange = (e: any) => {
    ...
    if ((name === "pincode") && type === "number" && value.length > 6) {
      return;
    }
  ...
  useEffect(() => {
    dispatch(setSlideTwoFormData(formData));
    console.log(combinedSlideTwo, "form 2 --");
  }, [formData, dispatch]);
  ...
  return (
    <Paper sx={{ padding: "20px" }}>
      <Grid container columnSpacing={3} marginTop={2}>
        ...
        <Grid item xs={12} md={6}>
          <FileUpload title="Address Proof" required={true} onFileUpload={handleAddressProofUpload} editFiles={''} />
        </Grid>
        <Grid item xs={12} md={6}>

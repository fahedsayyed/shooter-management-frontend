...
const FormSlideThree = () => {
  ...
  const { allTypeEvents, allMembershipPlans, allSubTypes, allMembershipPlansWithType, allMembershipSubTypesAllClubs } = useSelector((state: AppState) => state.utilFormSlice);
  const { athleteResponse, isLoading }: any = useSelector((state: AppState) => state.viewAthlete);
  const dispatch: AppDispatch = useDispatch();

  const {
    passport_number, passport_image, passport_issue_authority, date_of_issue, main, type, subtype, life_others,
    date_of_expiry, place_of_issue, firearms, coachDetails, bond_submission_date, payment_remark, idemnity_bond,
    validity, arjuna_awardee, arjuna_awardee_certificate, international_awardee, membership_number, international_awardee_certificate,
    membership_association_certificate,

  } = athleteResponse;


  const formattedDateOfIssue = formatDate(date_of_issue);
  const formattedDateOfExpiry = formatDate(date_of_expiry);
  const formattedDateOfValidity = formatDate(validity);
  const formattedDateOfBondSubmission = formatDate(bond_submission_date);
  // const formattedDateOfPaymentRemark = formatDate(payment_remark);

  // console.log(coachDetails, 'from coach details --');
  ...
  const initialFormData = {
    ...
    // passportDateAuthority: passport_issue_authority,
    main: main,
    type: type,
    subtype: subtype,
    selectLifeOthers: life_others,
    membershipNumber: membership_number,
    nraiShooterId: "",
    dateOfExpiry: formattedDateOfExpiry,
    passportPlaceIssue: place_of_issue,
    passportValidity: validity,
    arjunaAwardee: arjuna_awardee,
    internationalAwardee: international_awardee,
    arjunaAwardeeCertificate: arjuna_awardee_certificate,
    internationalAwardeeCertificate: international_awardee_certificate,
    membershipAssociationCertificate: membership_association_certificate,
    fireArms: firearms,
    coachDetails: coachDetails,
    bondSubmissionDate: formattedDateOfBondSubmission,
    // paymentRemark: formattedDateOfPaymentRemark,
    indemnityBond: idemnity_bond,
    validity: formattedDateOfValidity,
  } || {
    passportNumber: "",
    dateOfIssue: "",
  ...
  const [isPassportDisabled, setIsPassportDisabled] = useState(false);
  const [isPassportDatesDisabled, setIsPassportDatesDisabled] = useState(false);


  const updateFormData = useCallback(() => {
    dispatch(setSlideThreeFormData(formData));
    console.log(allMembershipSubTypesAllClubs, 'third clubs --');
    console.log(combinedSlideThree, "third form --");
  ...
  useEffect(() => {
    updateFormData();
    console.log(combinedSlideThree, "combined 2 data");
  }, [formData, dispatch]);

  useEffect(() => {
    const currentDate = new Date();
    const dateOfExpiry = new Date(formData.dateOfExpiry);

    const isPassportNumberEmpty = !formData.passportNumber;
    const isDateFieldsDisabled = isPassportNumberEmpty;
    const isDisabled = dateOfExpiry > currentDate;

    setIsPassportDisabled(isDisabled);
    setIsPassportDatesDisabled(isDateFieldsDisabled)
  }, [formData.passportNumber, formData.dateOfExpiry]);
    ...
    <Paper sx={{ padding: "20px" }}>
      <Grid container columnSpacing={3}>
        <Grid item xs={12} lg={6}>
          <CustomFormLabel htmlFor="bi-passport" sx={{ color: isPassportDisabled ? "#ccc" : "#000" }} >
            Passport Number
          </CustomFormLabel>
          <CustomTextField
            startadornment={
        ...
        <Grid item xs={12} lg={6}>
          <FileUpload title="passport" viewUploaded={passport_image} required={false} onFileUpload={handlePassportPhotoUpload} />
        </Grid>
        <Grid item xs={12} lg={6}>
        ...
        <Grid item xs={12} lg={6}>
          ...
          <CustomTextField
            ...
            onChange={handleChange}
            onBlur={() => checkError("placeOfIssue")}
            fullWidth
          />
        </Grid>
        <Grid item xs={12} lg={6}>
          ...
          <CustomTextField
            ...
            value={formData.passportIssueAuthority}
            onChange={handleChange}
            onBlur={() => checkError("passportIssueAuthority")}
            fullWidth
          />
        </Grid>
        <Grid item xs={12} lg={6}>
          ...
          <>
            ...
            <Grid item xs={12} lg={6}>
              <CustomFormLabel htmlFor="bi-subType">
                Name Of Rifle Club/District/Registered Units 
              </CustomFormLabel>
              <CustomSelect
                id="bi-subtype"
                name="subtype"
        ...
        <Grid item xs={12} lg={6}>
          <FileUpload title="Membership association certificate" required={true} viewUploaded={membership_association_certificate} onFileUpload={handleMembershiptAssociationUpload} />
        </Grid>
        <Grid item xs={12} lg={6}>
        ...
        </Grid>
        {
          formData.arjunaAwardee === "yes" && (
            <Grid item xs={12} lg={6}>
              <FileUpload title="Arjuna awardee certificate" required={false} viewUploaded={arjuna_awardee_certificate} onFileUpload={handleArjunaAwardeeUpload} />
            </Grid>
          )
        }
        {
          formData.internationalAwardee === "yes" && (
            <Grid item xs={12} lg={6} justifyContent='flex-end'>
              <FileUpload title="International awardee certificate" required={false} viewUploaded={international_awardee_certificate} onFileUpload={handleInternationalAwardeeUpload} />
            </Grid>
          )
        }
                    ...
                    <Grid container spacing={2} mt={0.3} key={i}>
                      <Grid item xs={12} lg={2}>
                        <FormControl fullWidth error={!!error.fireArms?.[i]?.weapon_type}>
                          ...
                          {error.fireArms?.[i]?.weapon_type && <FormHelperText> Weapon type is required.</FormHelperText>}
                        </FormControl>
                      </Grid>
                      ...
                      <Grid item xs={4} lg={4}>
                        <FormControl fullWidth>
                          <CustomTextField
                            ...
                            label="From Date"
                            name={`from_date`}
                            value={athleteResponse ? formatDate(coach.from_date) : coach.from_date}
                      ...
                      <Grid item xs={4} lg={4}>
                        <FormControl fullWidth>
                          <CustomTextField
                            ...
                            label="To Date"
                            name={`to_date`}
                            value={athleteResponse ? formatDate(coach.to_date) : coach.to_date}
                            // value={coach.to_date}
                            {...createFieldHandlers(i, 'coachDetails')}
                            type="date"
                            fullWidth
